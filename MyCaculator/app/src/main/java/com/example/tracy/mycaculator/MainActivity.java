package com.example.tracy.mycaculator;
...
public class MainActivity extends Activity implements OnClickListener{
    ...
    boolean firstFlag = true; //判断是否是首次输入,true首次,false不是首次
    boolean isClear = false; //是否清空显示区域的值,true需要,false不需要
    @Override
    ...
    public void onClick(View e) {
        ...
        if (firstFlag) {
            //判断首次输入的字符是否非法
            if (input.equals("÷")||input.equals("×")||input.equals("DEL")||input.equals("=")||input.equals("C")||input.equals(".")) {
                return ;
             }
            firstFlag = false;// 改变是否是首次输入的标记值
        }
        else  if(isClear &&(
